...
using RoR2.Skills;
using RoR2.UI;
using SecondMoon.AttackTypes.Orbs.Item.Void.TwistedRegrets;
...
public class LostBuff : Buff<LostBuff>
{
    ...
    public override void Hooks()
    {
        ...
        On.RoR2.GlobalEventManager.OnHitEnemy += AddDebuffsAndLaunchMissiles;
        IL.RoR2.HealthComponent.TakeDamage += SplitOsp;
    ...
    private void ChangeOspFractionDisplayCalc(ILContext il)
    {
        ...
        {
            ILLabel target = cursor.MarkLabel();
            if (cursor.TryGotoNext(MoveType.After, x => x.MatchStloc(2)))
            {
                ILLabel target2 = cursor.MarkLabel();
                if (cursor.TryGotoPrev(MoveType.After, x => x.MatchStloc(1)))
                {
    ...
    private void SetupBarColors(ILContext il)
    {
        ...
        if (cursor.TryGotoNext(x => x.MatchLdarg(0),
                               ...
                               x => x.MatchLdflda(typeof(HealthBar.BarInfoCollection), nameof(HealthBar.BarInfoCollection.shieldBarInfo))))
        {
            ILLabel target = cursor.MarkLabel();
            if (cursor.TryGotoNext(x => x.MatchLdloc(4),
                                   x => x.MatchLdloc(1),
                                   x => x.MatchLdfld(typeof(HealthComponent.HealthBarValues), nameof(HealthComponent.HealthBarValues.shieldFraction)),
                                   x => x.MatchLdloca(0)))
            {
                ILLabel target2 = cursor.MarkLabel();
                if (cursor.TryGotoPrev(x => x.MatchLdloc(1),
    ...
    private void AddDebuffsAndLaunchMissiles(On.RoR2.GlobalEventManager.orig_OnHitEnemy orig, GlobalEventManager self, DamageInfo damageInfo, GameObject victim)
    {
